...
type DocCategory = 'regulatory' | 'entity' | 'reference' | 'uploaded';
...
export default function GuidedAudit() {
  ...
  const totalDocs = regulatoryFiles.length + entityDocuments.length + referenceDocuments.length + uploadedDocuments.length;
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      ...
      if (uploadedWithText.length > 0) {
        ...
        const base = await analyzer.analyzeAssessment(assessment.data, regulatory, entity);
        result = {
  ...
  const handleRunRevisionCheck = async () => {
    ...
    try {
      ...
      setRevisionError(err instanceof Error ? err.message : 'Revision check failed');
    } finally {
        ...
        {step === 1 && (
          <>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {(['regulatory', 'entity', 'reference', 'uploaded'] as DocCategory[]).map((cat) => (
                <button
                  key={cat}
                  type="button"
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-sky to-sky-light rounded-xl font-semibold hover:shadow-lg hover:shadow-sky/30"
              >
                <FiUpload />
            ...
            <p className="text-white/40 text-xs mt-4">
              Documents: {totalDocs} total (regulatory: {regulatoryFiles.length}, entity: {entityDocuments.length},
          ...
          <>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-sky to-sky-light rounded-xl font-semibold disabled:opacity-50"
              >
                {analysisRunning ? <FiLoader className="animate-spin" /> : <FiFileText />}
          ...
          <>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-sky to-sky-light rounded-xl font-semibold disabled:opacity-50"
              >
                {simulationRunning ? <FiLoader className="animate-spin" /> : <FiUsers />}
          ...
          <>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-sky to-sky-light rounded-xl font-semibold disabled:opacity-50"
              >
                {revisionRunning ? <FiLoader className="animate-spin" /> : <FiRefreshCw />}
