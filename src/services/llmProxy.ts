...
export async function createMessage(params: CreateMessageParams): Promise<CreateMessageResponse> {
  ...
  for (let attempt = 0; attempt <= MAX_CLIENT_RETRIES; attempt++) {
    ...
    // Surface server's explanation for missing API keys or unreachable API
    if (response.status === 404) {
      lastError = new Error(
        'AI API is not available. For local development run: npm run dev:full (or use the deployed site).'
      );
    } else if (detail && (detail.includes('ANTHROPIC_API_KEY') || detail.includes('OPENAI_API_KEY'))) {
      lastError = new Error(
        detail + ' Open /api/ai-status in your browser to see what is configured on the server.'
      );
    } else if (response.status === 503) {
      lastError = new Error(
        (detail || 'AI not configured on server.') + ' Open /api/ai-status to check.'
      );
    } else {
