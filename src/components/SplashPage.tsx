...
function renderInlineMarkdown(text: string, cite?: CiteContext): Array<string | JSX.Element> {
...
function renderLightMarkdown(text: string, cite?: CiteContext): JSX.Element {
...
function AskSourcesPanel({
  ...
  const { citedTags } = segmentAnswerWithCitations(turn.content, sources);
  const cited = sources.filter((s) => citedTags.includes(s.tag));
  const uncitedCount = sources.length - cited.length;
        ...
        {turns.map((turn, i) => (
          <div key={`${turn.role}-${i}`} className={`flex ${turn.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              ...
              {turn.role === 'assistant' ? <AskSourcesPanel turn={turn} onOpenSource={onOpenSource} /> : null}
