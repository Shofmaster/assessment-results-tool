...
const AGENT_TYPES = AUDIT_AGENTS
  ...
  .map(a => ({
    ...
    color: a.color.includes('blue') ? 'text-blue-400'
      : a.color.includes('zinc') ? 'text-zinc-300'
      : a.color.includes('amber') ? 'text-amber-400'
      : a.color.includes('slate') ? 'text-slate-400'
      : a.color.includes('emerald') ? 'text-emerald-400'
      : a.color.includes('indigo') ? 'text-indigo-400'
      : a.color.includes('violet') ? 'text-violet-400'
      : a.color.includes('teal') ? 'text-teal-400'
      : a.color.includes('rose') ? 'text-rose-400'
      : a.color.includes('purple') ? 'text-purple-400'
      : a.color.includes('stone') ? 'text-stone-400'
      : a.color.includes('sky') ? 'text-sky-400'
      : a.color.includes('orange') ? 'text-orange-400'
      : a.color.includes('cyan') ? 'text-cyan-400'
      : a.color.includes('green') ? 'text-green-400'
      : a.color.includes('red') ? 'text-red-400'
      : a.color.includes('yellow') ? 'text-yellow-400'
      : a.color.includes('lime') ? 'text-lime-400'
      : a.color.includes('fuchsia') ? 'text-fuchsia-400'
      : a.color.includes('pink') ? 'text-pink-400'
      : 'text-white/60',
  }));
...
export default function AdminPanel() {
  ...
  // Get the _id → clerkUserId map for the selected user
  const selectedUserObj = useMemo(() => {
    return (allUsers || []).find((u: any) => u._id === togglesTargetUserId) ?? null;
  ...
  const toggleFramework = (fw: string) => {
    ...
    const next = current.includes(fw) ? current.filter(id => id !== fw) : [...current, fw];
    setDraftFrameworks(next);
    setTogglesDirty(true);
  };
    ...
    <div ref={containerRef} className="w-full min-w-0 p-3 sm:p-6 lg:p-8 h-full min-h-0">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            tab === 'toggles' ? 'bg-sky/20 text-sky-lighter border border-sky-light/30' : 'text-white/60 hover:text-white hover:bg-white/5'
          }`}
        >
            ...
            <>
                        ...
                        <div className="flex gap-1">
                          <button
                            onClick={() => {
                              ...
                              const current = draftAgents ?? ALL_NON_HOST_AGENT_IDS;
                              const next = [...new Set([...current, ...catIds])];
                              setDraftAgents(next); setTogglesDirty(true);
                            }}
                            className="text-[10px] px-2 py-0.5 rounded bg-green-500/10 text-green-400/70 hover:text-green-300"
                          >all on</button>
                          <button
                            onClick={() => {
                              const catIds = new Set<string>(agents.map(a => a.id));
                              const current = draftAgents ?? ALL_NON_HOST_AGENT_IDS;
                              setDraftAgents(current.filter(id => !catIds.has(id))); setTogglesDirty(true);
                            }}
              ...
              <GlassCard border rounded="xl">
                <div className="p-4 border-b border-white/10 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setDraftFrameworks(null); setTogglesDirty(true); }}
                      className="text-xs px-3 py-1.5 rounded-lg bg-green-500/15 text-green-300 hover:bg-green-500/25 border border-green-500/30"
                    >
                      Enable All
                    </button>
                    <button
                      onClick={() => { setDraftFrameworks([]); setTogglesDirty(true); }}
                      className="text-xs px-3 py-1.5 rounded-lg bg-red-500/15 text-red-300 hover:bg-red-500/25 border border-red-500/30"
                    >
                      Disable All
                    </button>
                    ...
                    <div key={cat}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex gap-1">
                          <button
                            onClick={() => {
                              const catIds = templates.map(t => t.framework);
                              const current = draftFrameworks ?? ALL_FRAMEWORK_IDS;
                              setDraftFrameworks([...new Set([...current, ...catIds])]); setTogglesDirty(true);
                            }}
                            className="text-[10px] px-2 py-0.5 rounded bg-green-500/10 text-green-400/70 hover:text-green-300"
                          >all on</button>
                          <button
                            onClick={() => {
                              const catIds = new Set(templates.map(t => t.framework));
                              const current = draftFrameworks ?? ALL_FRAMEWORK_IDS;
                              setDraftFrameworks(current.filter(id => !catIds.has(id))); setTogglesDirty(true);
                            }}
                            className="text-[10px] px-2 py-0.5 rounded bg-red-500/10 text-red-400/70 hover:text-red-300"
                          >all off</button>
                        </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                        {templates.map((tmpl) => {
                          ...
                          return (
                            <button
                              key={tmpl.framework}
                              onClick={() => toggleFramework(tmpl.framework)}
                              className={`flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${
                                enabled
                                  ? 'bg-white/5 border-white/15 hover:border-violet-400/30'
                                  : 'bg-white/[0.02] border-white/5 opacity-50 hover:opacity-70'
              ...
              <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10">
                <div>
                  {togglesDirty ? (
                    <p className="text-sm text-amber-300">Unsaved changes — click Save to apply.</p>
                  ) : (
                    <p className="text-sm text-white/50">No unsaved changes.</p>
                  )}
                </div>
                <button
                  onClick={handleSaveToggles}
                  disabled={!togglesDirty || togglesSaving}
                  className={`px-6 py-2 rounded-lg text-sm font-semibold transition-all ${
                    togglesDirty && !togglesSaving
