...
const parsedLibraryDocumentInValidator = v.object({
  contentHash: v.string(),
...
export const upsertParsedLibraryDctBatch = mutation({
  args: {
    companyId: v.id("companies"),
    documents: v.array(parsedLibraryDocumentInValidator),
  },
  handler: async (ctx, { companyId, documents }) => {
    await requireCompanyRole(ctx, companyId, [
      "company_admin",
      "company_manager",
    ]);
    const now = new Date().toISOString();
    ...
    for (const d of documents) {
      const existing = await ctx.db
        ...
        .withIndex("by_companyId_hash", (q: any) =>
          q.eq("companyId", companyId).eq("contentHash", d.contentHash),
        )
        .first();
      ...
      if (existing) {
        await ctx.db.patch(existing._id, {
          ...
          sourceSharedReferenceDocumentId:
            d.sourceSharedReferenceDocumentId ?? existing.sourceSharedReferenceDocumentId,
          updatedAt: now,
        });
      } else {
        await ctx.db.insert("dctParsedLibraryDocuments", {
          ...
          objective: d.objective,
          questionCount: d.questions.length,
          sourceSharedReferenceDocumentId: d.sourceSharedReferenceDocumentId,
          createdAt: now,
          updatedAt: now,
        });
      }
      ...
      const existingQs = await ctx.db
        ...
        .collect();
      for (const row of existingQs) await ctx.db.delete(row._id);
...
export const materializeProjectFromLibraryCache = mutation({
  args: { projectId: v.id("projects"), companyId: v.id("companies") },
  handler: async (ctx, { projectId, companyId }) => {
    ...
    const checkId = await ctx.db.insert("dctRevisionChecks", {
      ...
      kind: "xml_ingest",
      startedAt: now,
      summary: "Materialize DCT rows from company library cache",
    });

    const cacheDocs = await ctx.db
      .query("dctParsedLibraryDocuments")
    ...
    let materialized = 0;
    let alreadyReady = 0;
    let rebuiltMissing = 0;

    for (const cache of cacheDocs) {
      const cacheQuestions = await ctx.db
        .query("dctParsedLibraryQuestions")
        .withIndex("by_companyId_hash", (q: any) =>
          q.eq("companyId", companyId).eq("contentHash", cache.contentHash),
        )
        .collect();
      cacheQuestions.sort(
        (a: any, b: any) =>
      ...
      const expectedCount = cache.questionCount ?? cacheQuestions.length;
      const existingDoc = await ctx.db
        .query("dctToolDocuments")
        .withIndex("by_projectId_hash", (q: any) =>
          q.eq("projectId", projectId).eq("contentHash", cache.contentHash),
        )
        .first();

      if (existingDoc) {
        const counts = await countComparisonsForDoc(ctx, existingDoc._id);
        const fullyReady =
          counts.questionCount === expectedCount &&
          counts.comparisonCount === expectedCount;
        if (fullyReady) {
          alreadyReady++;
          continue;
        }
        await deleteQuestionsAndComparisonsForDoc(ctx, existingDoc._id);
        await ctx.db.patch(existingDoc._id, {
          source: "xml",
          fileName: cache.fileName,
          contentHash: cache.contentHash,
          standardDctId: cache.standardDctId,
          standardDctDetailId: cache.standardDctDetailId,
          dctVersionNumber: cache.dctVersionNumber,
          dctVersionDate: cache.dctVersionDate,
          dctStatus: cache.dctStatus,
          mlfId: cache.mlfId,
          mlfLabel: cache.mlfLabel,
          mlfName: cache.mlfName,
          assessmentTypeLabel: cache.assessmentTypeLabel,
          specialtyLabel: cache.specialtyLabel,
          peerGroupLabel: cache.peerGroupLabel,
    ...
    await ctx.db.patch(checkId, {
      completedAt: now,
      newOrUpdatedCount: materialized + rebuiltMissing,
      summary: `Materialized ${materialized} new, rebuilt ${rebuiltMissing}, already ready ${alreadyReady}`,
    });

    return {
      cacheCount: cacheDocs.length,
      materialized,
      rebuiltMissing,
      alreadyReady,
...
export const backfillParsedLibraryFromProject = mutation({
  args: { projectId: v.id("projects") },
  handler: async (ctx, { projectId }) => {
    await requireProjectOwner(ctx, projectId);
    const project = await ctx.db.get(projectId);
    if (!project?.companyId) {
      throw new Error("Project must belong to a company to backfill library cache.");
    }
    const docs = await ctx.db
      .query("dctToolDocuments")
      .withIndex("by_projectId", (q) => q.eq("projectId", projectId))
    ...
    for (const doc of docs) {
      ...
      const existingCache = await ctx.db
        .query("dctParsedLibraryDocuments")
        .withIndex("by_companyId_hash", (q: any) =>
          q.eq("companyId", project.companyId).eq("contentHash", doc.contentHash),
        )
        .first();
      if (existingCache) continue;
      const qs = await ctx.db
        .query("dctQuestions")
        .withIndex("by_dctDocumentId", (q: any) => q.eq("dctDocumentId", doc._id))
        .collect();
      await ctx.db.insert("dctParsedLibraryDocuments", {
